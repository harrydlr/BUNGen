...
| p              | `float` ∈ [0, 1], or `list` ∈ $[0,1]^{_$block\_number$_}$                              | noise outside a perfectly nested structure. If **p** is a `list` of length **block_number**, **p[α]** indicates the amount of this noise in block α. |
