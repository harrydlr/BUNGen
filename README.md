...
| p              | `float` ∈ [0, 1], or `list` ∈ $[0,1]^{\texttt{block_number}} $                             | noise outside a perfectly nested structure. If **p** is a `list` of length **block_number**, **p[α]** indicates the amount of this noise in block α. |
| mu             | `float` ∈ [0, 1]                                                                           | inter-block (i.e., between-modules) noise.                                                                                                           |
| y_block_nodes_vec | `list ∈ `$N^{\texttt{block_number}} $` \|` $\sum_{i=1}^{\texttt{block_number}} x_i$ = rows          | number of nodes per block in the y-axis.                                                                                                             |
| x_block_nodes_vec | `list ∈ `$N^{\texttt{block_number}} $` \|` $\sum_{i=1}^{\texttt{block_number}} x_i$ = cols | number of nodes per block in the x-axis.                                                                                                             |
