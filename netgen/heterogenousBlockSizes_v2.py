...
def heterogenousBlockSizes(B: int, N: int, min_block_size: int = 0, gamma: float = 2.5) -> List[int]:
    ...
    if gamma == 0:
        ...
        # Harry: warning when the min_block_size is not compatible with N and B
        if min_block_size > min([N//B + (ceil((N%B)/B) if (N%B-(b)>0) else 0) for b in range(B)]):
            warnings.warn(f"The minimum block size given the number of nodes and blocks can not be {min_block_size}. It will "
                  f"be set to {min([N//B + (ceil((N%B)/B) if (N%B-(b)>0) else 0) for b in range(B)])}")
            #print(f"The minimum block size given the number of nodes and blocks can not be {min_block_size}. It will "
            #      f"be set to {min([N//B + (ceil((N%B)/B) if (N%B-(b)>0) else 0) for b in range(B)])}")
        #
    ...
    clsum = 1
    maxn = N
    print(min([N//B + (ceil((N%B)/B) if (N%B-(b)>0) else 0) for b in range(B)]))

    res_nodes = N
    res_B = B
    colsizes = []
    for i_block in range(B):
        i_block_max_nodes = res_nodes - (res_B-1)
        i_block_nodes = stats.powerlaw.rvs(gamma, loc=1, scale=i_block_max_nodes, size=1)
        colsizes.append(i_block_nodes.round(0).astype(int))
        res_nodes = res_nodes-i_block_nodes
        res_B = res_B-(1+B)
    print("hola")
