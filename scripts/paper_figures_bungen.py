...
def printMatrix(M, Pij, crows, ccols, figName):
    ...
    plt.savefig(os.path.join(FIGURES_DIR, figName), dpi=720, bbox_inches='tight')
...
fig.savefig(os.path.join(FIGURES_DIR, 'fig1.jpg'), dpi=720, bbox_inches='tight')
...
plt.tight_layout()
plt.savefig(os.path.join(FIGURES_DIR, 'fig2.jpg'), dpi=720, bbox_inches='tight')
...
for x in range(fi):
    ...
    gen = NetworkGenerator(
        ...
        p=p,
        mu=mu,
        y_block_nodes_vec=[],  # equally-sized blocks
        x_block_nodes_vec=[],  # equally-sized blocks
        bipartite=True,
        fixedConn=True,
        link_density=1 / B)
...
plt.savefig(os.path.join(FIGURES_DIR, 'fig3.jpg'), dpi=720, bbox_inches='tight')
...
plt.savefig(os.path.join(FIGURES_DIR, 'fig4a.jpg'), dpi=720, bbox_inches='tight')
...
rect_ax2 = inset_axes(ax, width=2, height=2, bbox_to_anchor=(-0.045, .25, .1, .35),
                      bbox_transform=ax.transAxes, loc="lower left", borderpad=5)
rect_ax2.axis("off")
...
plt.savefig(os.path.join(FIGURES_DIR, 'fig4b.jpg'), dpi=720, bbox_inches='tight')
